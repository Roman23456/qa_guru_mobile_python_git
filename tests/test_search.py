...
def test_search_returns_results():
    ...
    with allure.step('Убедиться, что список результатов не пустой'):
        browser.all((AppiumBy.XPATH, '//androidx.compose.ui.platform.ComposeView'
                                     '//android.view.View[@clickable="true"]')).should(
...
def test_search_result_title_matches_query():
    ...
    with allure.step('Убедиться, что первый результат содержит слово "Appium"'):
        browser.all((AppiumBy.XPATH, '//androidx.compose.ui.platform.ComposeView'
                                     '//android.widget.TextView[@index="0"]')).first.should(
            have.text('Appium')
        )
